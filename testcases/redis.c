...
int main(int argc, char **argv) {
    redisContext *cc;
    redisReply *reply;
    const char *addr = (argc > 1) ? argv[1] : "127.0.0.1:6379";
    const char *testcase = (argc > 2) ? argv[2] : "setget";
    int keystart = (argc > 3) ? atoi(argv[3]) : 10000;
    int keyrange = (argc > 4) ? atoi(argv[4]) : 10000;
    ...
    if (port == NULL) {
        printf("invalid addr: %s", addr);
        return 1;
    } else {
        *port = '\0';
        port ++;
    }
    cc = redisConnectWithTimeout(addr, atoi(port), timeout);
    if (cc == NULL || cc->err) {
        if (cc) {
    ...
    if (strcmp(testcase, "setget") == 0) {
        for (count=1;;count++) {
            ...
            reply = redisCommand(cc, "SET %s %s", key, value);
            ...
            reply = redisCommand(cc, "GET %s", key);
            if (reply == NULL || reply->type == REDIS_REPLY_ERROR) {
                line = __LINE__;
                goto reply_error;
            }
        ...
        for (count=1;;count++) {
            ...
            sprintf(value, "%d", rand());
            reply = redisCommand(cc, "HSET %s %s %s", key, member, value);
            if (reply == NULL || reply->type == REDIS_REPLY_ERROR) {
                line = __LINE__;
                goto reply_error;
            }
            ...
            freeReplyObject(reply);
            reply = redisCommand(cc, "HGET %s %s", key, member);
            if (reply == NULL || reply->type == REDIS_REPLY_ERROR) {
                line = __LINE__;
                goto reply_error;
            }
        ...
        for (count=1;;count++) {
            ...
            sprintf(value, "%d", rand());
            reply = redisCommand(cc, "ZADD %s %s %s", key, value, member);
            if (reply == NULL || reply->type == REDIS_REPLY_ERROR) {
                line = __LINE__;
                goto reply_error;
            }
            assert(reply->type == REDIS_REPLY_INTEGER);
            freeReplyObject(reply);
            reply = redisCommand(cc, "ZRANK %s %s", key, member);
            if (reply == NULL || reply->type == REDIS_REPLY_ERROR) {
                line = __LINE__;
                goto reply_error;
            }
        ...
        for (count=1;;count++) {
            ...
            reply = redisCommand(cc, "SADD %s %s", key, member);
            if (reply == NULL || reply->type == REDIS_REPLY_ERROR) {
                line = __LINE__;
                goto reply_error;
            }
            assert(reply->type == REDIS_REPLY_INTEGER);
            freeReplyObject(reply);
            reply = redisCommand(cc, "SISMEMBER %s %s", key, member);
            if (reply == NULL || reply->type == REDIS_REPLY_ERROR) {
                line = __LINE__;
                goto reply_error;
            }
            ...
            if (reply->integer != 1) {
                line = __LINE__;
                sprintf(msg, "unexpected reply value %lld vs %d", reply->integer, 1);
                goto reply_fail;
            }
            freeReplyObject(reply);
        ...
        for (count=1;;count++) {
            sprintf(key, "%s_key_%d", testcase, keystart + count % keyrange);
            sprintf(value, "%d", rand());
            reply = redisCommand(cc, "LPUSH %s %s", key, value);
            if (reply == NULL || reply->type == REDIS_REPLY_ERROR) {
                line = __LINE__;
                goto reply_error;
            }
            assert(reply->type == REDIS_REPLY_INTEGER);
            freeReplyObject(reply);
            reply = redisCommand(cc, "LPOP %s", key);
            if (reply == NULL || reply->type == REDIS_REPLY_ERROR) {
                line = __LINE__;
                goto reply_error;
            }
        ...
        for (count=1;;count++) {
            ...
            reply = redisCommandArgv(cc, _argc, _argv, NULL);
            if (reply == NULL || reply->type == REDIS_REPLY_ERROR) {
                line = __LINE__;
                goto reply_error;
            }
        ...
        for (count=1;;count++) {
            sprintf(key, "%s_key_%d", testcase, keystart + count % keyrange);
            sprintf(value, "%d", rand());
            redisAppendCommand(cc, "SET %s %s", key, value);
            redisAppendCommand(cc, "GET %s", key);
            redisGetReply(cc, &reply);
            if (reply == NULL || reply->type == REDIS_REPLY_ERROR) {
                line = __LINE__;
                goto reply_error;
            }
        ...
        reply = redisCommandArgv(cc, argc-3, argv+3, NULL);
        if (reply == NULL) {
            line = __LINE__;
            goto reply_error;
        }
